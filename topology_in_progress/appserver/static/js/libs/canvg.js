...
(function(){
	...
	this.canvg = function (target, s, isStatic) {
		...
		if (s.substr(0,1) == '<') {
			...
			svg.loadXml(ctx, s, isStatic);
		}
		else {
			// load from url
			svg.load(ctx, s, isStatic);
		}
	}
	...
	function build() {
		...
		svg.CreateElement = function(node) {
		    if(node.nodeName=="text" && node.getAttribute("dy")){
		        node.setAttribute("y", parseFloat(node.getAttribute("y"))+parseFloat(node.getAttribute("dy")));
		    }
			var className = 'svg.Element.' + node.nodeName.toLowerCase().replace(/^[^:]+:/,'');
		...
		// load from url
		svg.load = function(ctx, url, isStatic) {
			svg.loadXml(ctx, svg.ajax(url), isStatic);
		}

		// load from xml
		svg.loadXml = function(ctx, xml, isStatic) {
			svg.init(ctx);
